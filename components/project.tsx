...
export default function Project({ title, description, tags, imageUrl, url }: ProjectProps) {
    ...
    return <>
            ...
            >
                <section
                className='relative rounded-lg bg-gray-100 max-w-[42rem] border border-black/5 overflow-hidden sm:pr-8 sm:h-[20rem] sm:group-even:pl-8 hover:bg-gray-200 transition dark:bg-white/10 dark:hover:bg-white/20'>
                <div className='sm:group-even:ml-[18rem] pt-4 pb-7 px-5 sm:pl-10  sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full'>
                    <h3 className='text-2xl font-semibold'>{title}</h3>
                    <p className='text-[0.84rem] my-2 leading-relaxed text-gray-700 dark:text-white/70'>{description}</p>
